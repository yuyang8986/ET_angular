...
@Component({
  selector: 'app-code38-p15-intangible-depreciating-assets-first-deducted',
  templateUrl: './code38-p15-intangible-depreciating-assets-first-deducted.component.html',
  styleUrls: ['./code38-p15-intangible-depreciating-assets-first-deducted.component.css']
})
export class Code38P15IntangibleDepreciatingAssetsFirstDeductedComponent implements OnInit {
  incomeTypeId: string = "";
    ...
    this.commonRequest.getTwoIds().subscribe((data) => {
      ...
      this.commonRequest.getMainForm(this.mainFormId).subscribe((infor) => {
        if (infor['incomeDetailsForm']){
          this.fetchData(infor['incomeDetailsForm']['incomeTypeDetails'], "38", "p15IntangibleDepreciatingAssetsFirstDeducted", "p15IntangibleDepreciatingAssetsFirstDeductedId") ///
        }
      })
    })
  }
