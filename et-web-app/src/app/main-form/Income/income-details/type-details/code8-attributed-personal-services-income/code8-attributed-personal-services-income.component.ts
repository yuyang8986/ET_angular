...
@Component({
  selector: 'app-code8-attributed-personal-services-income',
  templateUrl: './code8-attributed-personal-services-income.component.html',
  styleUrls: ['./code8-attributed-personal-services-income.component.css']
})
export class Code8AttributedPersonalServicesIncomeComponent implements OnInit {
    ...
    this.commonRequest.getTwoIds().subscribe((data) => {
      ...
      this.commonRequest.getMainForm(this.mainFormId).subscribe((infor) => {
        if (infor['incomeDetailsForm']){
          this.fetchData(infor['incomeDetailsForm']['incomeTypeDetails'], "8", "apsi", "apsiId") ///
        }
      })
    })
