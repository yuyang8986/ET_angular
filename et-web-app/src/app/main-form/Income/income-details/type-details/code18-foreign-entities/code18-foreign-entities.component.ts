...
@Component({
  selector: 'app-code18-foreign-entities',
  templateUrl: './code18-foreign-entities.component.html',
  styleUrls: ['./code18-foreign-entities.component.css']
})
export class Code18ForeignEntitiesComponent implements OnInit {
    ...
    this.commonRequest.getTwoIds().subscribe((data) => {
      ...
      this.commonRequest.getMainForm(this.mainFormId).subscribe((infor) => {
        if (infor['incomeDetailsForm']){
          this.fetchData(infor['incomeDetailsForm']['incomeTypeDetails'], "18", "foreignEntities", "foreignEntitiesId") ///
        }
      })
    })
  }
