...
@Component({
  selector: 'app-code9-bank-interest',
  templateUrl: './code9-bank-interest.component.html',
  styleUrls: ['./code9-bank-interest.component.css']
})
export class Code9BankInterestComponent implements OnInit {
    ...
    this.commonRequest.getTwoIds().subscribe((data) => {
      ...
      this.commonRequest.getMainForm(this.mainFormId).subscribe((infor) => {
        if (infor['incomeDetailsForm']){
          this.fetchData(infor['incomeDetailsForm']['incomeTypeDetails'], "9", "bankInterest", "bankInterestId") ///
        }
      })
    })
  }
