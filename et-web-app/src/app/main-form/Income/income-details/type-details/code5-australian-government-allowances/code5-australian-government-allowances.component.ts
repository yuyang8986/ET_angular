...
@Component({
  selector: 'app-code5-australian-government-allowances',
  templateUrl: './code5-australian-government-allowances.component.html',
  styleUrls: ['./code5-australian-government-allowances.component.css']
})
export class Code5AustralianGovernmentAllowancesComponent implements OnInit {
  subTitle: string = "Hi Nicky";
    ...
    this.commonRequest.getTwoIds().subscribe((data) => {
      ...
      this.commonRequest.getMainForm(this.mainFormId).subscribe((infor) => {
        if (infor['incomeDetailsForm']){
          this.fetchData(infor['incomeDetailsForm']['incomeTypeDetails'], "5", "aga", "AGAID") ///
        }
      })
    })
