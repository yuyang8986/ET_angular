...
@Component({
  selector: 'app-code21-bflicafs',
  templateUrl: './code21-bflicafs.component.html',
  styleUrls: ['./code21-bflicafs.component.css']
})
export class Code21BFLICAFSComponent implements OnInit {
    ...
    this.commonRequest.getTwoIds().subscribe((data) => {
      ...
      this.commonRequest.getMainForm(this.mainFormId).subscribe((infor) => {
        if (infor['incomeDetailsForm']){
          this.fetchData(infor['incomeDetailsForm']['incomeTypeDetails'], "21", "bflicafs", "BFLICAFSId") ///
        }
      })
    })
  }
