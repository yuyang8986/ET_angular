...
@Component({
  selector: 'app-code10-dividends',
  templateUrl: './code10-dividends.component.html',
  styleUrls: ['./code10-dividends.component.css']
})
export class Code10DividendsComponent implements OnInit {
  incomeTypeId: string = "";
    ...
    this.commonRequest.getTwoIds().subscribe((data) => {
      ...
      this.commonRequest.getMainForm(this.mainFormId).subscribe((infor) => {
        if (infor['incomeDetailsForm']){
          this.fetchData(infor['incomeDetailsForm']['incomeTypeDetails'], "10", "dividends", "dividendsId") ///
        }
      })
    })
  }
